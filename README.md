...
Deployment does **not** go through `docs/`. On each push to `main`, the
[deploy workflow](.github/workflows/deploy.yml) runs its own
`npx expo export -p web --output-dir dist`, adds `.nojekyll` and a `404.html`
SPA fallback, and uploads `src/MomsLifesaver/dist` straight to GitHub Pages.
The `docs/` folder is only a local preview target produced by
`npm run export:web`, and it is gitignored - nothing needs to be committed
for a deploy.
